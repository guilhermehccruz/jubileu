module.exports = {
	...
	async execute(servers, message, args) {
		...
		if (servers[message.guild.id].queue.length < 3) {
			return await message.channel.send('A fila não tem músicas pra trocar posições');
		}
		...
		if (isNaN(args[0]) || isNaN(args[1])) {
			return await message.channel.send('Apenas números são aceitos como argumentos');
		}
		...
		if (
			...
			return await message.channel.send('As posições precisam ser válidas');
		}

		if (args[0] == args[1]) {
			return await message.channel.send('As posições precisam ser diferentes');
		}
		...
		return await message.channel.send('Movido!');
	},
};
