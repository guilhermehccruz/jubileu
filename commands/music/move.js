module.exports = {
	...
	async execute(servers, message, args) {
		...
		if (servers[message.guild.id].queue.length < 3) {
			return message.reply('A fila não tem músicas pra trocar posições');
		}
		...
		if (isNaN(args[0]) || isNaN(args[1])) {
			return message.reply('Apenas números são aceitos como argumentos');
		}
		...
		if (
			...
			return message.reply('As posições precisam ser válidas');
		}

		if (args[0] == args[1]) {
			return message.reply('As posições precisam ser diferentes');
		}
